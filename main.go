...
import (
	...
	"git.fe-cred.com/idfor/idfor/consensus"
	...
	hserver "git.fe-cred.com/idfor/idfor/http/base/actor"
	"git.fe-cred.com/idfor/idfor/http/jsonrpc"
	"git.fe-cred.com/idfor/idfor/http/localrpc"
	"git.fe-cred.com/idfor/idfor/http/nodeinfo"
	"git.fe-cred.com/idfor/idfor/http/restful"
	"git.fe-cred.com/idfor/idfor/http/websocket"
	"git.fe-cred.com/idfor/idfor/p2pserver"
	netreqactor "git.fe-cred.com/idfor/idfor/p2pserver/actor/req"
	p2pactor "git.fe-cred.com/idfor/idfor/p2pserver/actor/server"
	"git.fe-cred.com/idfor/idfor/txnpool"
	tc "git.fe-cred.com/idfor/idfor/txnpool/common"
	"git.fe-cred.com/idfor/idfor/txnpool/proc"
	"git.fe-cred.com/idfor/idfor/validator/stateful"
...
func startDNA(ctx *cli.Context) {
	...
	log.Infof("DNA version %s", config.Version)
	...
	if err != nil {
		log.Errorf("initExecutor error:%s", err)
		return
	}
	...
	p2pSvr, p2pPid, err := initP2PNode(ctx, txpool, acc)
	if err != nil {
...
func initConfig(ctx *cli.Context) (*config.DNAConfig, error) {
	//init DNA config from cli
	cfg, err := cmd.SetDNAConfig(ctx)
	if err != nil {
		return nil, err
	}
...
func initAccount(ctx *cli.Context) (*account.Account, error) {
	...
	executorFile := ctx.GlobalString(utils.GetFlagName(utils.ExecutorFileFlag))
	if executorFile == "" {
		return nil, fmt.Errorf("Please config executor file using --executor flag")
	}
	if !common.FileExisted(executorFile) {
		return nil, fmt.Errorf("Cannot find executor file:%s. Please create executor first", executorFile)
	}
...
func initP2PNode(ctx *cli.Context, txpoolSvr *proc.TXPoolServer, acc *account.Account) (*p2pserver.P2PServer, *actor.PID, error) {
	if config.DefConfig.Genesis.ConsensusType == config.CONSENSUS_TYPE_SOLO {
		return nil, nil, nil
	}
	p2p := p2pserver.NewServer()
	if acc != nil {
		p2p.SetAddr(acc.Address.ToBase58())
	}
