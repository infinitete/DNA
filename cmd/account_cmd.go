...
func accountCreate(ctx *cli.Context) error {
	...
	executor, err := account.Open(optionFile)
	if err != nil {
		return fmt.Errorf("open executor error:%s", err)
	}
	...
	for i := 0; i < optionNumber; i++ {
		...
		acc, err := executor.NewAccount(label, keyType, curve, scheme, pass)
		if err != nil {
			return fmt.Errorf("new account error:%s", err)
		}
		PrintInfoMsg("Index:%d", executor.GetAccountNum())
...
func accountList(ctx *cli.Context) error {
	...
	executor, err := account.Open(optionFile)
	if err != nil {
		return fmt.Errorf("open executor:%s error:%s", optionFile, err)
	}
	accNum := executor.GetAccountNum()
	if accNum == 0 {
	...
	for i := 0; i < ctx.NArg(); i++ {
		addr := ctx.Args().Get(i)
		accMeta := common.GetAccountMetadataMulti(executor, addr)
		if accMeta == nil {
			PrintWarnMsg("Cannot find account by:%s in executor:%s", addr, utils.GetFlagName(utils.ExecutorFileFlag))
			continue
		}
		accList[accMeta.Address] = ""
	}
	for i := 1; i <= accNum; i++ {
		accMeta := executor.GetAccountMetadataByIndex(i)
		if accMeta == nil {
			continue
		}
		if len(accList) > 0 {
...
func accountSet(ctx *cli.Context) error {
	...
	executor, err := common.OpenExecutor(ctx)
	if err != nil {
		return err
	}
	accMeta := common.GetAccountMetadataMulti(executor, address)
	if accMeta == nil {
	...
	if ctx.Bool(utils.GetFlagName(utils.AccountSetDefaultFlag)) {
		err = executor.SetDefaultAccount(address)
		if err != nil {
	...
	if ctx.IsSet(utils.GetFlagName(utils.AccountLabelFlag)) {
		...
		err = executor.SetLabel(address, newLabel)
		if err != nil {
	...
	if ctx.IsSet(utils.GetFlagName(utils.AccountSigSchemeFlag)) {
		...
		if find {
			err = executor.ChangeSigScheme(address, sigSch)
			if err != nil {
	...
	if ctx.Bool(utils.GetFlagName(utils.AccountChangePasswdFlag)) {
		...
		// verify old password
		if _, err := executor.GetAccountByAddress(address, passwd); err != nil {
		...
		err = executor.ChangePassword(address, passwd, newPass)
		if err != nil {
...
func accountDelete(ctx *cli.Context) error {
	...
	defer common.ClearPasswd(passwd)
	_, err = executor.DeleteAccount(accMeta.Address, passwd)
	if err != nil {
...
func accountImport(ctx *cli.Context) error {
	...
	if source == "" {
		PrintErrorMsg("Missing source executor path argument to import.")
		cli.ShowSubcommandHelp(ctx)
		return nil
	}

	target := ctx.String(utils.GetFlagName(utils.ExecutorFileFlag))
	fn := checkFileName(ctx)
	executor, err := account.Open(fn)
	if err != nil {
		return err
	}
	...
	importPri := func(pri keypair.PrivateKey, pwd []byte) {
		...
		old := executor.GetAccountMetadataByAddress(b58addr)
		if old != nil {
		...
		err = executor.ImportAccount(&accMeta)
		if err != nil {
	...
	if ctx.Bool(utils.GetFlagName(utils.AccountWIFFlag)) {
		...
		ctx.Set(utils.GetFlagName(utils.ExecutorFileFlag), source)
		sourceExecutor, err := common.OpenExecutor(ctx)
		if err != nil {
			return fmt.Errorf("open source executor to import error: %s", err)
		}
		accountNum := sourceExecutor.GetAccountNum()
		if accountNum == 0 {
		...
		for i := 0; i < ctx.NArg(); i++ {
			addr := ctx.Args().Get(i)
			accMeta := common.GetAccountMetadataMulti(sourceExecutor, addr)
			if accMeta == nil {
				PrintWarnMsg("Cannot find account %s in executor %s.", addr, source)
				continue
			}
			accList[accMeta.Address] = ""
		}
		...
		for i := 1; i <= accountNum; i++ {
			accMeta := sourceExecutor.GetAccountMetadataByIndex(i)
			if accMeta == nil {
				continue
			}
			if len(accList) > 0 {
			...
			total++
			old := executor.GetAccountMetadataByAddress(accMeta.Address)
			if old != nil {
				skip++
				PrintWarnMsg("Account: %s (label: %s) already exists in executor, skip.", accMeta.Address, accMeta.Label)
				continue
			}
			err = executor.ImportAccount(accMeta)
			if err != nil {
...
func accountExport(ctx *cli.Context) error {
	...
	client, err := common.OpenExecutor(ctx)
	if err != nil {
		return err
	}
	executor := client.GetExecutorData()
	if ctx.IsSet(utils.GetFlagName(utils.AccountLowSecurityFlag)) {
		...
		err := executor.ToLowSecurity(passwords)
	...
	err = executor.Save(target)
	if err != nil {
		return fmt.Errorf("save executor file error: %s", err)
	}

	PrintInfoMsg("Export executor success.")
	return nil
}
