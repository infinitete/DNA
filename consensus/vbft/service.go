...
import (
	...
	actorTypes "git.fe-cred.com/idfor/idfor/consensus/actor"
	...
	gover "git.fe-cred.com/idfor/idfor/smartcontract/service/native/governance"
	ninit "git.fe-cred.com/idfor/idfor/smartcontract/service/native/init"
	nutils "git.fe-cred.com/idfor/idfor/smartcontract/service/native/utils"
	"git.fe-cred.com/idfor/idfor/validator/increment"
	...
	case msg := <-self.msgC:
		...
		case BlockProposalMessage:
			...
			if msgBlkNum == self.GetCurrentBlockNo() {
				...
				if self.isProposer(msgBlkNum, pMsg.Block.getProposer()) {
					...
					// stop proposal timer
					self.timer.CancelProposalTimer(msgBlkNum)
		...
		case BlockEndorseMessage:
			...
			if msgBlkNum == self.GetCurrentBlockNo() {
				...
				self.blockPool.newBlockEndorsement(pMsg)
				...
				if self.isEndorser(msgBlkNum, pMsg.Endorser) {
					...
					if proposer, forEmpty, done := self.blockPool.endorseDone(msgBlkNum, self.config.C); done {
						// stop endorse timer
						self.timer.CancelEndorseMsgTimer(msgBlkNum)
						// stop empty endorse timer
						self.timer.CancelEndorseEmptyBlockTimer(msgBlkNum)
		...
		case BlockCommitMessage:
			...
			if msgBlkNum == self.GetCurrentBlockNo() {
				...
				if proposer, forEmpty, done := self.blockPool.commitDone(msgBlkNum, self.config.C, self.config.N); done {
					...
					// stop commit timer
					self.timer.CancelCommitMsgTimer(msgBlkNum)
